...
Shader "Custom/ScreenSpaceReflection3"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            /// </summary>
            bool isRayHit(float3 startRayWS, out float2 rayUV, out int hitFlag)
            {
                ...
                float2 uv)
            {
                ...
                for (int n = 1; n <= _StepCount; n++)
                {
                    ...
                    if (isRayHit(startRayWS, rayUV, hitFlag))
                    {
                        hitUV = rayUV;
                        break;
                    }
            ...
            half4 frag(Varyings IN) : SV_Target
            {
                ...
                {
                    float rawDepth = SAMPLE_DEPTH_TEXTURE(_CameraDepthTexture, sampler_PointClamp, hitUV);
                    float depth = Linear01Depth(rawDepth, _ZBufferParams); // 手前0,奥1
